...
通过配置化数据和分层封装降低维护成本；
通过 AI 辅助分析，把"执行完拿到的结果"快速转化为"可读的归因报告和补测建议"。
```

核心取舍是：后台系统真正的风险通常不在页面样式，而在接口权限、异步状态、设备回调、数据一致性和日志可追踪性。因此本方案优先投入接口自动化、权限矩阵、OTA 全链路和数据一致性校验，UI 自动化只覆盖后台真实操作主流程。在测试执行完成后，引入 AI 辅助分析层，自动读取 Allure 结果、失败用例和模块分布，生成失败归因、优先排查路径和补测建议，让"测试执行 → 结果分析 → 用例补全"形成闭环，而不是跑完就结束。
...
| L7 AI 辅助分析层 | 读取 Allure 结果、失败用例、模块分布，调用 LLM 生成归因报告 | 把"执行结果"快速转化为"可读归因 + 补测建议"，闭环测试价值 |

L1~L6 解决"怎么测"，L7 解决"测完之后怎么快速消化结果"。L7 不是替代人工判断，而是把人工需要花 30 分钟翻报告才能得出的初步结论，压缩到一次模型调用内输出，让人工只做最终确认。
...
flowchart TD
    ...
    J --> K["日志查询和审计校验"]
    K --> L["核心 UI 链路测试"]
    L --> M["生成 Allure 报告和失败定位信息"]
    M --> O["L7: AI 读取结果生成归因报告 + 补测建议"]
    O --> P["人工审核确认并沉淀回归用例"]
    P --> N["接入 CI/CD 分层回归"]
```
...
| AI 辅助分析 | DashScope qwen3.7-plus（OpenAI 兼容接口） | 第三阶段 | 读取 Allure 结果自动生成归因报告和补测建议，无需额外模型运维 |
...
AI 在本方案中不是一个独立的"AI 测试模块"，而是嵌入到自动化测试的三个关键环节里，分别解决"测前怎么设计、测后怎么消化结果、日常怎么维护"这三个最容易吃人力的问题。最终判断仍由测试人员结合真实业务和环境确认，AI 负责把重复性高、耗时但价值不低的工作压缩成秒级输出。

### 12.1 AI 在测试流程中的三个融入点

| 融入点 | 阶段 | AI 做什么 | 提效价值 | 对应实现 |
|---|---|---|---|---|
| ① 用例与 Mock 设计 | 测试前 | 根据接口文档/需求生成 pytest 模板、service 封装、权限矩阵数据、Mock 设备脚本 | 用例冷启动从天级降到小时级 | AI 辅助生成 + 人工审核 |
| ② 失败归因与补测建议 | 测试后 | 读取 Allure 结果，分类失败原因，输出优先排查路径和补测用例建议 | 把"翻报告 30 分钟"压缩到一次调用 | `ai_assistant/analyze_results.py` |
| ③ 框架维护提示 | 日常 | 根据接口变更提示影响范围、识别用例倾斜（如权限模块仅 3 条） | 防止维护滞后和覆盖盲区 | AI 分析报告中的"框架维护建议"章节 |

### 12.2 融入点②：失败归因链路（已落地）

本项目中 AI 不只停留在文档描述，还设计了一个可选的 AI 辅助分析链路：自动化用例执行后，脚本读取 Allure/pytest 结果，整理失败用例、模块分布、执行步骤和错误信息；如果在 `.env` 中配置了 `MODEL_API_KEY`、`MODEL_API_BASE` 和 `MODEL_NAME`，会通过 DashScope/OpenAI 兼容的 `chat/completions` 接口调用 `qwen3.7-plus`，生成失败原因分类、优先排查路径、补充回归用例建议和框架维护建议。如果没有配置 API Key，则生成规则版分析报告，保证流程可演示、可落地。
